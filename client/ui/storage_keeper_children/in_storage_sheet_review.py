...
class InStorageSheetReview():
    ...
    def commit(self):
        ...
        if (statue == 0):
            ...
            statue, data = self.storager_keeper.add_in_storage_sheet(id=id, warehouse_name=warehouse_name, date=date)
            if (statue == False):
                QtWidgets.QMessageBox.information(None, '提示', '提交入库总单出现错误，请重试！', QtWidgets.QMessageBox.Ok)
                return False
            ...
            for i, item in enumerate(self.form_data['details']):
                ...
                number = item['number']
                statue, data = self.storager_keeper.add_in_storage_sheet_details(id=details_id, in_storage_sheet_id=id,
